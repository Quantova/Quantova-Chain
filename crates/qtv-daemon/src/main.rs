...
fn guard_derived_keys(dev: bool) {
    ...
    eprintln!(
        "\nquantovad refuses to start.\n\n\
         This build derives every validator's key material from one devnet master secret\n\
         and the validator id. Whoever holds that master can name a validator id and so\n\
         derive that validator's whole identity, its sortition tree, its attestation key and\n\
         its peer identity. It is a chain one master holder owns entirely, fine on that\n\
         holder's own machines and unacceptable the moment a second party runs a node,\n\
         because the second party's identity would be one the master holder can forge.\n\n\
         Pass --dev to run under this master derived key model on your own boxes. Operator\n\
         held keys, one secret per validator generated by that validator and held in its own\n\
         keystore, with only the commitments published, are the required next step before\n\
